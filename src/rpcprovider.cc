...
json基于文本存储，而json是键值对存储，需要有个键值    protobuf基于二进制存储，而且是紧密存储不携带除数据外任何其他信息，
                            需要的带宽资源更少，带宽资源传输数据量更大，不仅支持序列化和反序列化，还提供一些抽象服务的方法  
  protobuf效率更高
*/
//这里是框架提供给外部使用的，可以发布rpc方法的函数接口，所以我们必须站在抽象的角度看问题（可以接收任意的service），不能站在某一个业务层看问题
...
void RpcProvider::Run()
{
    ...
    //绑定连接回调和消息读写回调方法    好处：分离了网络代码和业务代码
    ...
    //std::cout << "RpcProvidr start service at ip:" << ip <<" port:" << port << std::endl;
    LOG_INFO("RpcProvidr start service at ip:%s port:%d", ip.c_str(),port);

    //启动网络服务
    server.start();
    m_eventLoop.loop();//相当于启动epoll wait以阻塞方式等待远程连接
}
...
/*
在框架内部，Rpcprovider和RpcConsumer协商好之间通信使用的protobuf数据类型
service_name method_name args   定义proto的message类型，进行数据头的序列化和反序列化
                                service_name method_name args_size(防止参数连包问题)
                            ...
                            muduo::Timestamp)
{
    ...
    recv_buf.copy((char*)&header_size, 4, 0);//从第0位置开始拷贝4个字节内容到recv_buf中
    ...
    {
        //数据头反序列化成功
        service_name = rpcHeader.service_name();//生成读方法 定义proto时，会生成数据对象的读写方法
